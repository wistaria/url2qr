...
- Box Access Token: Box パス workflow で使用
...
Box workflow を使う場合は、Box token も設定します。
...
## Box Access Token の取得方法

1. [Box Developer Console](https://app.box.com/developers/console) にサインインします。
2. Box app を作成、または既存の app を開きます。
3. フォルダ読み取りと共有リンク管理に必要な権限を有効にします。
4. 新しい access token を生成します。
5. シェルに設定します。
...
Box 権限不足に関するエラーが表示される場合、Box app token に十分なアクセス権がありません。

1. Box Developer Console を開き、対象 app を選択します。
2. フォルダと共有リンクに必要な権限を有効にします。
3. 変更を保存します。
4. 新しい access token を生成します。
5. 新しい token を export して、もう一度 Box workflow を実行します。
